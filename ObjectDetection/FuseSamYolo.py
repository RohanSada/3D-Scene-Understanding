...
class ObjectDetector3D:
    def __init__(self, calibration_file='./utils/camera_calibration.json', stride=4):
        ...
        self.object_detector = YoloInstanceSeg()
    ...
    def process_frame(self, frame):
        ...
        for det in detections:
            ...
            spatial_objects.append({
                ...
                "pcd_points": pcd_points_np,
                # Enable these two only for test runs (only for running this file)
                #"bbox_o3d": obb, 
                #"pcd": pcd
            })
...
class YoloInstanceSeg():
    def __init__(self, model_path='./models/yolov8s-seg.pt'):
        """
        Initializes the single YOLOv8 instance segmentation model.
        Ensure you download a segmentation model (ending in -seg.pt).
        """
        self.model = YOLO(model_path)
        self.erode_kernel = np.ones((5, 5), np.uint8) 

    def _get_iou(self, box, mask_box):
        # Kept for compatibility, though not needed for single-model inference
        # since the box and mask come from the same prediction.
        return 1.0

    def process_frame(self, frame, erosion_iterations=5):
        """
        Runs YOLOv8 Segmentation.
        Returns the exact same dictionary structure as FuseSamYolo.
        """
        # Run inference (retina_masks=True gives better quality masks)
        results = self.model(frame, verbose=False, retina_masks=True)
        
        fused_results = []
        result = results[0] # We are only processing one frame
        ...
        if result.masks is not None:
            ...
            # Extract masks (H, W) matches the original frame size because of retina_masks=True
            # If standard masks are returned, they might be smaller, so we resize below just in case.
            ...
            for i, (box, cls, conf) in enumerate(zip(boxes, clss, confs)):
                ...
                # Get the specific mask for this object
                # Note: raw_masks[i] might be smaller (e.g. 640x640) if retina_masks=False
                # or native resolution if retina_masks=True. We resize to be safe.
                mask_raw = raw_masks[i]
                
                # Resize to match original frame dimensions if necessary
                if mask_raw.shape[:2] != (h, w):
                    mask_raw = cv2.resize(mask_raw, (w, h), interpolation=cv2.INTER_LINEAR)
                
                # Convert to binary uint8 mask
                mask_uint8 = (mask_raw > 0.5).astype(np.uint8)

                # Apply Erosion (matching your previous logic)
                ...
                result_entry = {
                    ...
                    "box": box.tolist(),
                    # For a single model, the mask box is the same as the detection box
                    "mask_box": box.tolist(), 
                    "mask": processed_mask,
                    # IoU is 1.0 because the mask and box are generated by the same head
                    "iou": 1.0 
                }
    ...
    def display_fused_results(self, frame, fused_results):
        """
        Visualizes the results. Exact same logic as your previous class.
        """
...
if __name__=='__main__':
    ...
    processor = ObjectDetector3D(calibration_file='../utils/camera_calibration.json', stride=4)
